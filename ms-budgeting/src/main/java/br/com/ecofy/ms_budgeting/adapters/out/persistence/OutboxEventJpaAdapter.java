...
import br.com.ecofy.ms_budgeting.adapters.out.persistence.repository.OutboxEventRepository;
...
public class OutboxEventJpaAdapter implements OutboxEventPort {
    ...
    private static OutboxEvent toDomain(OutboxEventEntity e) {
        return new OutboxEvent(e.getId(), e.getAggregateType(), e.getAggregateId(), e.getEventType(),
                e.getEventVersion(), e.getTopic(), e.getPartitionKey(), e.getPayload(), e.getCorrelationId(),
                e.getCausationId(), e.getStatus(), e.getAttempts(), e.getNextAttemptAt(), e.getOccurredAt(),
                e.getCreatedAt(), e.getUpdatedAt(), e.getProcessingStartedAt(), e.getPublishedAt(),
                e.getLastErrorCode(), e.getLastErrorAt());
    }
}
