...
public interface OutboxEventRepository extends JpaRepository<OutboxEventEntity, UUID> {
    ...
    @Query("""
            select e
             ...
             where e.status = br.com.ecofy.ms_categorization.core.domain.outbox.OutboxStatus.PENDING
                or (e.status = br.com.ecofy.ms_categorization.core.domain.outbox.OutboxStatus.FAILED
    ...
    // Recupera registros presos em PROCESSING além do timeout de processamento.
    @Query("""
            select e
              from OutboxEventEntity e
             where e.status = br.com.ecofy.ms_categorization.core.domain.outbox.OutboxStatus.PROCESSING
               and e.processingStartedAt < :threshold
            """)
    List<OutboxEventEntity> findStuckProcessing(@Param("threshold") Instant threshold);

    // Remove registros PUBLISHED em lote limitado, preservando FAILED e DISCARDED para auditoria.
    @Modifying
    @Query(value = """
            DELETE FROM cat_outbox_events
             WHERE id IN (
                   SELECT id FROM cat_outbox_events
    ...
    @Query(value = """
            SELECT MIN(created_at) FROM cat_outbox_events
    ...
    @Query("""
            select count(e)
              from OutboxEventEntity e
             where e.status = br.com.ecofy.ms_categorization.core.domain.outbox.OutboxStatus.DISCARDED
            """)
    long countDiscarded();
}
