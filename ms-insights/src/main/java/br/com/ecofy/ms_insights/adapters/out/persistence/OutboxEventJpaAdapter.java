...
import br.com.ecofy.ms_insights.adapters.out.persistence.entity.OutboxEventEntity;
import br.com.ecofy.ms_insights.adapters.out.persistence.repository.OutboxEventRepository;
...
// Persiste e gerencia o ciclo de processamento dos eventos da Outbox.
@Slf4j
@Component
...
public class OutboxEventJpaAdapter implements OutboxEventPort {
    ...
    // Persiste o evento na transação corrente do caso de uso.
    @Override
    ...
    public void save(OutboxEvent event) {
        ...
        log.debug(
                "[OutboxEventJpaAdapter] - [save] -> outbox gravada eventId={} eventType={} aggregateId={}",
                event.getId(),
                event.getEventType(),
    ...
    // Reserva eventos elegíveis sem prolongar o bloqueio durante a publicação.
    @Override
    @Transactional
    public List<OutboxEvent> claimPendingBatch(int batchSize, Instant now) {
        List<OutboxEventEntity> locked = repository.lockEligibleBatch(
                now,
                batchSize
        );
        if (locked.isEmpty()) {
    ...
    public int releaseStuck(Duration timeout, Instant now) {
        ...
        List<OutboxEventEntity> stuck =
                repository.findStuckProcessing(threshold);
        ...
        if (!stuck.isEmpty()) {
            log.warn(
                    "[OutboxEventJpaAdapter] - [releaseStuck] -> {} registros presos em PROCESSING liberados",
                    stuck.size()
            );
        }
        return stuck.size();
    ...
    @Override
    @Transactional
    public int deletePublishedOlderThan(
            Duration retention,
            Instant now,
            int maxRows
    ) {
        return repository.deletePublishedBatch(
                now.minus(retention),
                maxRows
        );
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public long countStuckProcessing(Duration timeout, Instant now) {
