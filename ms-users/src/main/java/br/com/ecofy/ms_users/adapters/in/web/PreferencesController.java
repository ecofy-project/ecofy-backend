...
@Validated
@Tag(name = "Users - Preferences", description = "Consulta e atualização de preferências do usuário")
...
public class PreferencesController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "Parâmetros inválidos / payload inválido"),
            ...
            @ApiResponse(responseCode = "500", description = "Erro interno ao atualizar preferências")
    })
    ...
    public ResponseEntity<UserPreferencesResponse> update(
        ...
        Objects.requireNonNull(req.preferences(), "req.preferences must not be null");

        Map<PreferenceKey, String> parsed = parsePreferences(req.preferences());
        ...
        var result = updateUseCase.update(new UpdatePreferencesCommand(userId, parsed, idempotencyKey));
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "401", description = "Não autenticado (JWT ausente/inválido)"),
            @ApiResponse(responseCode = "404", description = "Usuário não encontrado (se sua regra assim definir)"),
            @ApiResponse(responseCode = "500", description = "Erro interno ao buscar preferências")
    })
    @GetMapping("/{userId}")
    public ResponseEntity<UserPreferencesResponse> get(@PathVariable("userId") @NotNull UUID userId) {
        log.debug("[PreferencesController] - [get] -> userId={}", userId);
    ...
    /**
     * Política atual: ignora chaves inválidas (forward-compatible).
     * Se preferir "fail-fast", troque o try/catch por exception -> 400 via ControllerAdvice.
     */
    private static Map<PreferenceKey, String> parsePreferences(Map<String, String> raw) {
        ...
        for (Map.Entry<String, String> e : raw.entrySet()) {
            ...
            try {
                PreferenceKey key = PreferenceKey.valueOf(rawKey.trim());
                map.put(key, value);
            } catch (IllegalArgumentException ex) {
                // Ignora chaves desconhecidas (ou altere para erro 400 conforme sua política)
                log.debug("[PreferencesController] - [parsePreferences] -> ignoring unknown preferenceKey={}", rawKey);
            }
        }
